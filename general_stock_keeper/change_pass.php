...
if(isset($_POST['btn_submit'])){
    ...
    $get_database_password = mysqli_query($connection,"SELECT * FROM $corrected_users_table WHERE `users_table_id` = '$get_user_id' LIMIT 1");
    if(mysqli_num_rows($get_database_password) >= 1){
        ...
        if(password_verify($get_old_password,$get_my_pass)){
            ...
            $run_update_query = mysqli_query($connection,"UPDATE $corrected_users_table SET `user_pass` = '$strong_pass' WHERE $corrected_users_table.`users_table_id` = '$get_user_id' ");
            if($run_update_query){
