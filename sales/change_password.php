...
$my_users_table = $_COOKIE['b']."_users_table";
$my_user_id = $_COOKIE['i'];
...
if(isset($_POST['btn_submit'])){
    ...
    $get_database_password = mysqli_query($connection,"SELECT * FROM $my_users_table WHERE `users_table_id` = '$my_user_id' LIMIT 1");
    if(mysqli_num_rows($get_database_password) >= 1){
        ...
        if(password_verify($get_old_password,$get_my_pass)){
            ...
            $run_update_query = mysqli_query($connection,"UPDATE $my_users_table SET `user_pass` = '$strong_pass' WHERE $my_users_table.`users_table_id` = '$my_user_id' ");
            if($run_update_query){
